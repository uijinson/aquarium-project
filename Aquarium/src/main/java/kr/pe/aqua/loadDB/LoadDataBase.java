...
public class LoadDataBase {
	...
	public CommandLineRunner MemberInitDatabase(MemberRepository repository) {
		return args -> {
			...
			log.info("Preloading " + repository.save(Member.builder().memId("1").pw(1).nickName("1").build()));
		};
	}
	...
	public CommandLineRunner FishinItDatabase(FishRepository repository) {
		...
		return args -> {
			log.info("Preloading " + repository.save(Fish.builder().fishName("shark").fishExplain("wicked2").fishImg("/img/fish/shark.jpg").build()));
			log.info("Preloading " + repository.save(Fish.builder().fishName("nimo").fishExplain("nimo").fishImg("/img/fish/nimo.jpg").build()));
		};
	}
	...
	public CommandLineRunner EquipmentInitDatabase(EquipmentRepository repository) {
		return args -> {
			...
			log.info("Preloading " + repository
					.save(Equipment.builder().equipName("gun2").equipExplain("gun2").equipImg("/img/equip/gun2.jpg").equipPrice(100).build()));
		};
	}
